...
lm_sby = lm(sby_need ~ pmax(0,(calls - 9200)), data = filter(ts_sby, date >= "2018-01-01"))
...
system.time(
  progressr::with_progress(
    sby_basemodel <- ts_sby %>%
      filter(date < "2018-12-01", !is.na(MA2_8)) %>%
      model(
...
system.time(
  progressr::with_progress(
    sby_basefc <- sby_basemodel %>%
      forecast(h = "2 months", level = c(95))
  )
)
```
