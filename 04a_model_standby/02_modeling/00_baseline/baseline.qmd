...
mq <- function(x, q = 0.9, frame = 7) {
  ...
  for (i in 1:length(x)) {
    ifelse((i - frame) <= 0,
           value <- 0,
           value <- quantile(x[(i - frame):i], q))
    y <- c(y, value)
  }
  return(y)
}

combined <- singlecosts %>% 
  filter(model %in% c("ma", "snaive", "ets", "ets_season")) %>%
  group_by(date, value) %>% summarise(comb_pred = max(predicted))

mq <- data %>% 
  mutate(mq = mq(value, q = 0.999, frame = 4)) %>% 
  select(date,
         value = mq)

p <- ggplot(data = combined) + 
  geom_line(aes(x = date, y = value), colour = "black") +
  geom_line(aes(x = date, y = comb_pred), colour = "red") +
  geom_line(data = mq, aes(x = date, y = value), colour = "blue")
