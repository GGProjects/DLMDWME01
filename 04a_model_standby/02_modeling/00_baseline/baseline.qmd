...
train.prophet <- function(x, y, wts, param, lev, last) {
  df <- cbind(x, y = y)
  m <- prophet(df)
  return(list(model = m))
}
...
prophet_model <- train(
  ...
  tuneGrid = expand.grid(changepoint_prior_scale = 0.1)
)
...
-   `trainControl(method = "timeslice")` sorgt für Rolling-Origin-CV.

-   `initialWindow = 150` bedeutet, dass die ersten 150 Tage fürs Training genutzt werden.

-   `horizon = 10` bedeutet, dass das Modell jeweils 10 Tage in die Zukunft vorhersagt.

-   `fixedWindow = FALSE` erlaubt ein sich erweiterndes Trainingsfenster.

-   `caret::train()` trainiert das `prophet`-Modell unter Verwendung der Cross-Validation.

```{r}
# Parameter für Cross-Validation
initial <- 70  # Größe des initialen Trainingssets
horizon <- 10  # Vorhersagehorizont
period <- 10   # Schrittweite für die Verschiebung des Trainingsfensters

# Zeitbasierte Cross-Validation Splits mit caret
time_slices <- caret::createTimeSlices(1:nrow(df), initialWindow = initial, horizon = horizon, fixedWindow = FALSE)
...
# Cross-Validation Setup mit der eigenen Loss-Funktion
control <- trainControl(method = "cv", number = 5, 
                        summaryFunction = custom_loss)

# Prophet-Modell mit eigener Loss-Funktion und Cross-Validation
prophet_model <- train(
  y ~ ., 
  data = df, 
  method = "prophet", 
